					...
					struct inode *inode)
{
	if (!inode)
		return true;
/*ZTE_MODIFY start, we use resuid and resgid to control apps access root reserved blocks*/
#if 0
...
/*ZTE_MODIFY end*/
	if (IS_NOQUOTA(inode))
		return true;
/*ZTE_MODIFY start, allow specified files to use reserved blocks*/
	if (file_use_reserved_block(inode))
		return true;
/*ZTE_MODIFY end*/
/*ZTE_MODIFY start, allow specified process to use reserved blocks*/
	if (uid_gte(F2FS_OPTION(sbi).s_resuid, current_fsuid()) ||
					gid_gte(F2FS_OPTION(sbi).s_resgid, current_fsgid()))
/*ZTE_MODIFY end*/
		return true;
