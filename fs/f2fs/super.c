...
/*ZTE_MODIFY start, we use resuid and resgid mount options to control app access root reserved blocks
 * the root reserved blocks can be configured when formatting or can be enable with tunef2fs at boot time
 */
#if 0
	if (!test_opt(sbi, RESERVE_ROOT) &&
...
try_onemore:
	...
	F2FS_OPTION(sbi).s_resuid = make_kuid(&init_user_ns, le16_to_cpu(raw_super->s_def_resuid));
	F2FS_OPTION(sbi).s_resgid = make_kgid(&init_user_ns, le16_to_cpu(raw_super->s_def_resgid));
	/*ZTE_MODIFY end*/

	/*ZTE_MODIFY add root reserved blocks support*/
	F2FS_OPTION(sbi).root_reserved_blocks = le32_to_cpu(raw_super->s_resv_segments);
	/*ZTE_MODIFY end*/
	/* parse mount options */
