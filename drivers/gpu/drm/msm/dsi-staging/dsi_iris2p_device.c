 ...
 *
 * Copyright (C) 2016-2017 Pixelworks Inc.
...
int parse_auto_args(char *s, struct auto_args *args)
{
	...
	args->argv = kmalloc(args->argc * sizeof(struct auto_arg),
					GFP_KERNEL);
...
/*
 * factory bp test
 * enable iris bp mode: adb echo bp 1 > /dev/iris2p
 * enable iris bp mode with hbm enabled: adb echo bp 2 > /dev/iris2p
 * disable iris bp mode: adb echo bp 0 > /dev/iris2p
 *
 */
...
int do_cmd_te_opt(struct iris2p_data *pdata, const struct auto_args *args)
{
	...
	if (value) {
		pr_err("%s:enabling panel te tweaking mode\n", __func__);
	} else {
		pr_err("%s: disabling panel te tweaking mode\n", __func__);
	}
...
int do_cmd_dump_opt(struct iris2p_data *pdata, const struct auto_args *args)
{
	...
	if (value) {
		pr_err("%s:enabling panel dsi packets dump\n", __func__);
	} else {
		pr_err("%s: disabling panel dsi packets dump\n", __func__);
	}
