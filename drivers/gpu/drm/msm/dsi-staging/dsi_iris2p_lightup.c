...
static int iris_panel_te_tweak_enabled = 1;

static int iris_bp_mode = IRIS_NO_ABYPASS;

static int iris_dump_dsi_packets_enabled = 0;
...
#define KINGDISPLAY_TE_ONE_STEP_US          (17)
...
static int iris_trace_printout_opt = 0;

int iris_full_functioning = -1;
int iris_in_cont_splash_displaying = -1;

static int zte_lcd_module = 0;/*add by zte for distinguish lcd module*/
...
static int iris_lightup_failure_recover_simulated = 0;
...
{
	iris_lightup_failure_recover_simulated = opt;
}

bool irisIsSimluatedResetFailureRecover(void)
{
	return (iris_lightup_failure_recover_simulated&RECOVER_RESET_FAILURE_SIMULATED);
}

bool irisIsSimluatedEnableFailureRecover(void)
{
	return (iris_lightup_failure_recover_simulated&RECOVER_ENABLE_FAILURE_SIMULATED);
}

bool irisIsSimluatedInitFailureRecover(void)
{
	return (iris_lightup_failure_recover_simulated&RECOVER_INIT_FAILURE_SIMULATED);
}
...
static bool isAllowedLightupTrace(void)
{
...
{
	return (iris_info.abypss_ctrl.abypass_status == ANALOG_BYPASS_MODE);
}

bool irisIsInAbyassHbmEn(void)
{
	return (iris_bp_mode == IRIS_IN_ABYPASS_HBM_ENABLED);
}

void irisSetAbyassMode(int mode)
{
	iris_bp_mode = mode;
}
...
void iris_timing_init(struct dsi_iris_info *iris_info)
{
	...
	if (zte_lcd_module == LCM_KINGDISPLAY) {
		pr_info("iris KINGDISPLAY timing\n");
		pinput_timing->hres = 1080;
		pinput_timing->hfp = 120;
		pinput_timing->hbp = 36;
		pinput_timing->hsw = 4;
		pinput_timing->vres = 2248;
		pinput_timing->vbp = 8;
		pinput_timing->vfp = 60;
		pinput_timing->vsw = 4;
		pinput_timing->fps = 60;

		poutput_timing->hres = 1080;
		poutput_timing->hfp = 120;
		poutput_timing->hbp = 36;
		poutput_timing->hsw = 4;
		poutput_timing->vres = 2248;
		poutput_timing->vbp = 8;
		poutput_timing->vfp = 129;
		poutput_timing->vsw = 4;
		poutput_timing->fps = 60;
	} else {
		pr_info("iris SAMSUNG timing\n");
		pinput_timing->hres = 1080;
		pinput_timing->hfp = 100;
		pinput_timing->hbp = 32;
		pinput_timing->hsw = 16;
		pinput_timing->vres = 2248;
		pinput_timing->vbp = 8;
		pinput_timing->vfp = 10;
		pinput_timing->vsw = 2;
		pinput_timing->fps = 60;

		poutput_timing->hres = 1080;
		poutput_timing->hfp = 100;
		poutput_timing->hbp = 32;
		poutput_timing->hsw = 16;
		poutput_timing->vres = 2248;
		poutput_timing->vbp = 8;
		poutput_timing->vfp = 78;
...
void iris_clock_tree_init(struct dsi_iris_info *iris_info)
{
	...
	u8 clk_default[6] = {0x01, 0x01, 0x03, 0x01, 0x03, 0x72};
	u8 indx;

	if (zte_lcd_module == LCM_KINGDISPLAY) {
		pr_info("iris KINGDISPLAY clock tree\n");
		pll->ppll_ctrl0 = 0x2;
		pll->ppll_ctrl1 = 0x251101;
		pll->ppll_ctrl2 = 0x8b3663;

		pll->dpll_ctrl0 = 0x2002;
		pll->dpll_ctrl1 = 0x251101;
		pll->dpll_ctrl2 = 0x78D50;
	} else {
		pr_info("iris SAMSUNG clock tree\n");
		pll->ppll_ctrl0 = 0x2;
		pll->ppll_ctrl1 = 0x241101;
		pll->ppll_ctrl2 = 0x5b0a0f;

		pll->dpll_ctrl0 = 0x2002;
		pll->dpll_ctrl1 = 0x231101;
		pll->dpll_ctrl2 = 0xdb8bac;
	}
...
void iris_mipi_info_init(struct dsi_iris_info *iris_info)
{
	...
	if (zte_lcd_module == LCM_KINGDISPLAY) {
		pr_info("iris KINGDISPLAY mipi info\n");
		...
		tx_setting->mipitx_dsi_tx_ctrl = 0xA004139;
		tx_setting->mipitx_hs_tx_timer = 0x879B27;
		tx_setting->mipitx_bta_lp_timer = 0x00ffff37;
		tx_setting->mipitx_initialization_reset_timer = 0x0a8c08AD;
		tx_setting->mipitx_dphy_timing_margin = 0x00040401;
		tx_setting->mipitx_lp_timing_para = 0x0F010006;
		tx_setting->mipitx_data_lane_timing_param = 0x140c1206;
		tx_setting->mipitx_clk_lane_timing_param = 0x140b2e08;
		tx_setting->mipitx_dphy_pll_para = 0x000008ad;
		tx_setting->mipitx_dphy_trim_1 = 0xedb5380c;
	} else {
		pr_info("iris SAMSUNG mipi info\n");
		rx_setting->mipirx_dsi_functional_program = 0x64;
		rx_setting->mipirx_eot_ecc_crc_disable = 7;
		rx_setting->mipirx_data_lane_timing_param = 0xff09;

		tx_setting->mipitx_dsi_tx_ctrl = 0xA004139;
		tx_setting->mipitx_hs_tx_timer = 0x83507f;
		tx_setting->mipitx_bta_lp_timer = 0x00ffff37;
		tx_setting->mipitx_initialization_reset_timer = 0x0a8c0867;
		tx_setting->mipitx_dphy_timing_margin = 0x00040401;
		tx_setting->mipitx_lp_timing_para = 0x0f010006;
		tx_setting->mipitx_data_lane_timing_param = 0x140c1106;
		tx_setting->mipitx_clk_lane_timing_param = 0x140b2c08;
		tx_setting->mipitx_dphy_pll_para = 0x00000867;
		tx_setting->mipitx_dphy_trim_1 = 0xedb5380c;
	}
}
...
#define DELTA_PERIOD_MAX 68
	...
	dtg_setting->dvs_ctrl = sw_dvs_period;//for cinvout,should + (2 << 30);
...
{
	struct iris_abypass_ctrl *pabypss_ctrl = &(iris_info->abypss_ctrl);

	pabypss_ctrl->analog_bypass_enable = true;
...
void iris_info_init(struct dsi_panel *panel)
{
	...
	/*add by zte for distinguish lcd module*/
	if (!strcmp(iris_panel->name, "KINGDISPLAY-RM69299-1080-2248-6P2Inch")) {
		zte_lcd_module = LCM_KINGDISPLAY;
	} else {
		zte_lcd_module = LCM_SAMSUNG;
	}
	/*add by zte for distinguish lcd module end*/
				...
				enum dsi_cmd_set_state state)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (panel->host == NULL || cmds == NULL) {
			pr_err("iris failed to set cmds: invalid panel->host or cmds\n");
			break;
		}
		...
		if (len < 0) {
			rc = len;
			pr_err("iris failed to set cmds(%d), rc=%d\n", cmds->msg.type, rc);
			goto error;
		}
...
u8 iris_mipi_power_mode_read(struct dsi_panel *panel, enum dsi_cmd_set_state state)
{
	...
	const struct mipi_dsi_host_ops *ops = panel->host->ops;
	struct dsi_cmd_desc cmds = {
		{0, MIPI_DSI_DCS_READ, 0, 0, 0, 1, (const void *)get_power_mode, 1, (void *)iris_read_cmd_buf}, 1, 0};
	int rc = 0;
	u8 data = 0xff;
...
int iris_Romcode_state_check(struct dsi_panel *panel, u8 data, u8 retry_cnt)
{
	...
	/* read failed */
	if (cnt == retry_cnt) {
		pr_err("iris_Romcode_state_check retrycnt=%d,powermode %x failed in %x\n", retry_cnt, data, powermode);
		return -EINVAL;
	}
#endif
	return 0;
...
void iris_init_cmd_send(struct dsi_panel *panel, enum dsi_cmd_set_state state)
{
	struct dsi_cmd_desc iris_init_info_cmds[] = {
		{{0, MIPI_DSI_GENERIC_LONG_WRITE, 0, 0, CMD_PROC,
			CMD_PKT_SIZE, init_cmd[0].cmd, 1, iris_read_cmd_buf},
			1, CMD_PROC},
		{{0, MIPI_DSI_GENERIC_LONG_WRITE, 0, 0, MCU_PROC * 2,
			CMD_PKT_SIZE, init_cmd[1].cmd, 1, iris_read_cmd_buf},
			1, MCU_PROC * 2},
		{{0, MIPI_DSI_GENERIC_LONG_WRITE, 0, 0, MCU_PROC * 2,
			CMD_PKT_SIZE, init_cmd[2].cmd, 1, iris_read_cmd_buf},
			1, MCU_PROC * 2},
	};
...
void iris_timing_info_send(struct dsi_panel *panel, enum dsi_cmd_set_state state)
{
	...
	struct dsi_cmd_desc iris_timing_info_cmd[] = {
		{{0, MIPI_DSI_GENERIC_LONG_WRITE, 0, 0, MCU_PROC,
			sizeof(iris_workmode), iris_workmode, 1, iris_read_cmd_buf},
			1, MCU_PROC},
		{{0, MIPI_DSI_GENERIC_LONG_WRITE, 0, 0, MCU_PROC,
			sizeof(iris_timing), iris_timing, 1, iris_read_cmd_buf},
			1, MCU_PROC},
	};
	struct iris_timing_info *pinput_timing = &(iris_info.input_timing);
	struct iris_timing_info *poutput_timing = &(iris_info.output_timing);
...
void iris_ctrl_cmd_send(struct dsi_panel *panel, u8 cmd, enum dsi_cmd_set_state state)
{
	...
	struct dsi_cmd_desc iris_romcode_ctrl_cmd = {
		{0, MIPI_DSI_GENERIC_LONG_WRITE, 0, 0, 0,sizeof(romcode_ctrl), romcode_ctrl, 1, iris_read_cmd_buf}, 1, CMD_PROC};
...
void iris_grcp_buffer_init(struct dsi_panel *panel, enum dsi_cmd_set_state state)
{
	...
	struct dsi_cmd_desc iris_grcp_buf_init_cmd = {
		{0, MIPI_DSI_GENERIC_LONG_WRITE, 0, 0, 0,sizeof(grcp_buf_init), grcp_buf_init, 1, iris_read_cmd_buf}, 1, CMD_PROC};
					...
					enum dsi_cmd_set_state state)
{
	...
	case MIPI_DSI_DCS_READ://0x06
		...
		if (irisIsAllowedDsiTrace())
			pr_info("iris: dcs handover header(shortread): 0x%x\n", header.hdr32);
		iris_cmd_reg_add(&grcp_cmd, IRIS_MIPI_TX_ADDR + WR_PACKET_HEADER_OFFS, header.hdr32);
		break;
	...
	case MIPI_DSI_GENERIC_LONG_WRITE://0x29
		...
		for (cnt = 0; cnt < panel_dcs_cmd->msg.tx_len; cnt = cnt + 4) {
			if ((panel_dcs_cmd->msg.tx_len - cnt) >= 4) {
				...
				memcpy(payload.p,
					((u8 *)panel_dcs_cmd->msg.tx_buf) + cnt,
					panel_dcs_cmd->msg.tx_len - cnt);
			}
			if (irisIsAllowedDsiTrace())
				pr_err("iris: dcs handover payload(longwrite): 0x%x\n", payload.pld32);
...
int iris_handover_dcs_panel_switch_dimming(struct dsi_panel *panel, u16 brightness, enum dsi_cmd_set_state state)
{
	...
	{0, MIPI_DSI_DCS_SHORT_WRITE_PARAM, 0, 0,0, 
		sizeof(displayControlPayload), displayControlPayload,
		1, iris_read_cmd_buf},
		1, CMD_PROC};
	...
	if (displayControlPayload[1] == 0x28) {
		if (irisIsInAbypassMode() || irisIsInLowpowerAbypassMode()) {
			...
			/*iris_dsi_cmds_send(panel, &panel_dcs_control_display_cmd, 1, state);*/ /*modify by zte temp*/
			ret = iris_handover_panel_dcs_cmd(panel, &panel_dcs_control_display_cmd, state);
		}
		if (isAllowedLightupTrace())
			pr_info("iris: setting the 53-register for display dimming returns %d", ret);
...
int iris_handover_dcs_set_display_brightness(struct dsi_panel *panel, u16 brightness, enum dsi_cmd_set_state state)
{
	...
	{0, MIPI_DSI_DCS_LONG_WRITE, 0, 0,0, 
		sizeof(brightnessPayload), brightnessPayload,
		1, iris_read_cmd_buf},
		1, CMD_PROC};
	...
	{0, MIPI_DSI_DCS_LONG_WRITE, 0, 0,0, 
		sizeof(brightnessPayload_8bit), brightnessPayload_8bit,
		1, iris_read_cmd_buf},
		1, CMD_PROC};
	...
	if (!panel->panel_initialized || os_bootup_completed == 0) {
		...
		pr_info("iris_handover_dcs_set_display_brightness: ignored, os bootup is still ongoing [%d, %d, %d]\n",
			(int)panel->panel_initialized, (int)iris_in_cont_splash_displaying, (int)iris_full_functioning);
		return ret;
	}

	if (!record_first_kickoff) {
		//if the first kickoff is not triggered, ignore the backlight change.
	...
	if (!record_first_backlight) {
		...
		if (timeus01 < BACKLIGHT_ON_AFTER_XMS_FIRST_KICKOFF*1000) {
			...
			if (timeus01 < BACKLIGHT_ON_AFTER_XMS_FIRST_KICKOFF*1000/2)
				pr_info("%s:lpabypass=%d,brightness=%d,hbm=%d,first backlight on at %d, after kickoff for %dus\n",
					__func__,
					iris_info.abypss_ctrl.abypass_status,
					brightness,
					panel_hbm_on, timeus1, timeus01);
		}
	}

	/*modify by fuchun for managing Display Dimming start*/
#ifdef CONFIG_ZTE_LCD_DCSBL_CABC_GRADIENT
	iris_handover_dcs_panel_switch_dimming(panel, brightness, state);
	/*modify by fuchun for managing Display Dimming end*/
#endif

	lastBrightness = brightness;

	/*modify by yujianhua for analog bypass mode start*/
	if (isAllowedLightupTrace())
		pr_info("%s:status=%d,brightness=%d\n", __func__, iris_info.abypss_ctrl.abypass_status, brightness);
	if (irisIsInAbypassMode() || irisIsInLowpowerAbypassMode()) {
	/*modify by yujianhua for analog bypass mode end*/
		if (panel_hbm_on)
			return ret;
		/*modify by yujianhua for lcd bl start*/
	...
	/*modify by yujianhua for lcd bl start*/
	if (panel->bl_config.bl_max_level > 255) {
		/*iris_dsi_cmds_send(panel, &panel_dcs_brightness_cmd, 1, state);*/ /*modify by zte temp*/
		ret = iris_handover_panel_dcs_cmd(panel, &panel_dcs_brightness_cmd, state);
	} else {
		/*iris_dsi_cmds_send(panel, &panel_dcs_brightness_cmd_8bit, 1, state);*/ /*modify by zte temp*/
...
void iris_prepare_for_hdr_switch(struct dsi_panel *panel, int hdr_en)
{
	if (hdr_en) {
		...
		if (panel->bl_config.bl_max_level == lastBrightness) {
			/*modify by yujianhua for lcd bl start*/
			iris_handover_dcs_set_display_brightness(panel, panel->bl_config.bl_max_level,
...
void iris_feature_init_cmd_send(struct dsi_panel *panel, enum dsi_cmd_set_state state)
{
	...
	if (!iris_info.lut_info.lut_fw_state || !iris_info.gamma_info.gamma_fw_state) {
		//if ccf files were not loaded, try loading again.
...
#ifdef EFUSE_WRITE
		iris_cmd_reg_add(&grcp_cmd, IRIS_SYS_ADDR + DFT_EFUSE_CTRL, 0x00000000);
		iris_cmd_reg_add(&grcp_cmd, IRIS_SYS_ADDR + DFT_EFUSE_CTRL_1, 0x4000600d);
...
#ifdef READ_CMD_ENABLE
			if (!strcmp(panel->name, "zteSAM_S6E3FA3_SAM_1080x2248_6P2Inch_video_nolcd")) {
				pr_info("%s no lcd, not check firmware download result!\n", __func__);
				result = true;
...
void iris_firmware_cmlut_update(u8 *buf, u32 cm3d)
{
	...
	if (!lut_info->lut_fw_state) {
		pr_err("iris_firmware_cmlut_update: cm lut read failed\n");
		return;
	}

	if (isAllowedLightupTrace())
		pr_err("iris_firmware_cmlut_update: copy cm3d(%d)\n", cm3d);
...
void iris_efuse_write(struct dsi_panel *panel)
{
		...
		*(u16 *)(grcp_cmd.cmd + 14) = cpu_to_le16(grcp_len);
		...
		pr_err("iris_pre_lightup efusewrite sending: msg length=%d\n", (int)iris_efusewrite_cmd.msg.tx_len);
		iris_dsi_cmds_send(panel, &iris_efusewrite_cmd, 1, DSI_CMD_SET_STATE_LP);
		pr_err("iris_pre_lightup efusewrite sent\n");
		//DUMP_PACKET(grcp_cmd.cmd, grcp_cmd.cmd_len);
}
#endif
...
{
	pr_err("iris triggering ccf fw loading ...\n");
...
void iris_firmware_gamma_table_update(u8 *buf, u32 cm3d)
{
	...
	if (!iris_info.gamma_info.gamma_fw_state || !gamma_fw_buf) {
		pr_err("iris_firmware_gamma_table_update: ccf not ready");
		return;
	}
...
void iris_firmware_download_cont_splash(struct dsi_panel *panel)
{
	...
	if (iris_debug_fw_download_disable) {
		...
		pr_info("%s iris_debug_fw_download_disable!\n", __func__);
	} else {
#if 0
		/*should not request ccf here if panel calibration enabled in sf*/
		iris_lut_firmware_init(LUT_FIRMWARE_NAME);
		iris_gamma_firmware_init(GAMMA_FIRMWARE_NAME);
#endif
		/* wait Romcode ready */
		if (!strcmp(panel->name, "zteSAM_S6E3FA3_SAM_1080x2248_6P2Inch_video_nolcd")) {
			pr_info("%s no lcd, not state check powermode=3!\n", __func__);
		} else {
			ret = iris_Romcode_state_check(panel, 0x03, 50);
			pr_info("%s iris_Romcode_state_check ret=%d!\n", __func__, ret);
			if (ret < 0 || irisIsSimluatedEnableFailureRecover())
...
void iris_dport_enable_cmd_send(struct dsi_panel *panel, bool enable)
{
	...
	struct dsi_cmd_desc dport_enable_cmd = {
		{0, MIPI_DSI_GENERIC_LONG_WRITE, 0, 0,0, 
			sizeof(dport_enable), dport_enable,
			1, iris_read_cmd_buf},
			1, 16};
...
void iris_pre_lightup(struct dsi_panel *panel)
{
	...
	ktime0 = ktime_get();
	panel_hbm_on = false;
	iris_full_functioning = 0;/*reset flag*/
	iris_in_cont_splash_displaying = 0;
...
#ifdef EFUSE_WRITE
	pr_err("iris_pre_lightup efuse writing\n");
	iris_mipirx_mode_set(panel, PWIL_CMD, DSI_CMD_SET_STATE_LP);
	iris_efuse_write(panel);
	usleep_range(10000, 10010);
	pr_err("iris_pre_lightup efuse written\n");
#endif

	ret = iris_Romcode_state_check(panel, 0x01, 50);
	if (ret < 0 || irisIsSimluatedResetFailureRecover())
	...
	if (isAllowedLightupTrace())
		pr_err("iris_pre_lightup iris_timing_info_sent\n");
	iris_mipirx_mode_set(panel, PWIL_CMD, DSI_CMD_SET_STATE_LP);
	if (isAllowedLightupTrace())
		pr_err("iris_pre_lightup iris_timing_info_sent\n");
	iris_ctrl_cmd_send(panel, CONFIG_DATAPATH, DSI_CMD_SET_STATE_LP);
	if (isAllowedLightupTrace())
		pr_err("iris_pre_lightup iris_ctrl_cmd_sent\n");
	/*init grcp buffer*/
	iris_grcp_buffer_init(panel, DSI_CMD_SET_STATE_HS);
	if (isAllowedLightupTrace())
		pr_err("iris_pre_lightup iris_grcp_buffer_init_sent\n");
	/*init feature*/
	iris_feature_init_cmd_send(panel, DSI_CMD_SET_STATE_HS);
	if (isAllowedLightupTrace())
		pr_err("iris_pre_lightup iris_feature_init_sent\n");
	/* bypass panel on command */
	iris_mipirx_mode_set(panel, BYPASS_CMD, DSI_CMD_SET_STATE_LP);
	if (isAllowedLightupTrace())
		pr_err("iris_pre_lightup iris_BYPASS_CMD_sent\n");

	if (iris_info.work_mode.tx_mode == DSI_OP_CMD_MODE) {
		u32 column, page;

		column = iris_info.work_mode.rx_ch ? (iris_info.input_timing.hres * 2 - 1)
										: (iris_info.input_timing.hres - 1);
		page = iris_info.input_timing.vres - 1;
...
void iris_lightup(struct dsi_panel *panel)
{
...
#ifdef SAMSUNG_PANEL_TE_ADJUST_ALTERNATIVE
	if (zte_lcd_module == LCM_SAMSUNG) {
		if (panel_expected_c0 == panel_working_c0
			&& panel_working_c0 != 0) {
			set_te_on_next_wakeup = 1;
			pr_info("iris_lightup: will re-use the trained panel te on-wakeup: 0x%x\n", panel_expected_c0);
		}

		if (set_te_on_next_wakeup > 0) {
			panel_working_c0 = 0;
		...
		if (iris_panel_te_tweak_enabled > 0
			&& set_te_on_next_wakeup > 0) {
			panelTeCalibrationRequired = 1;
			set_te_on_next_wakeup = 0;
			pr_info("iris_lightup: set panel te on-wakeup: 0x%x\n", panel_expected_c0);
			validatePanelExpectedC0();
			irisAdjustPanelTE(panel, 1, panel_expected_c0, DSI_CMD_SET_STATE_LP);
			usleep_range((10000), 10010);
		}
	} else {
		panel_working_c0 = 0;
	...
	if (iris_debug_fw_download_disable) {
		...
		/* wait Romcode ready */
		ret = iris_Romcode_state_check(panel, 0x03, 50);
		if (ret < 0 || irisIsSimluatedInitFailureRecover())
	...
	pr_info("iris light up time delay: %d %d %d\n", timeus0, timeus1, timeus2);

	if (irisStayInLowPowerAbypass()) {
		pr_info("iris light up into low power abypass\n");
...
{
	pr_info("iris_lightdown reset status.\n");
	iris_info.abypss_ctrl.abypass_status = PASS_THROUGH_MODE;
	iris_info.abypss_ctrl.abypass_to_pt_enable = 0;
	iris_info.abypss_ctrl.pt_to_abypass_enable = 0;
	iris_full_functioning = 0;
...
u8 irisReadPanelTeConfig_Samsung(struct dsi_panel *panel, enum dsi_cmd_set_state state)
{
	...
	struct dsi_cmd_desc cmds[] = {
		...
		{{0, MIPI_DSI_DCS_READ, BIT(6), 0, 0,
			sizeof(get_c0_value), get_c0_value,
			1, iris_response_c0},
			1, 0},
	...
	if (irisIsInAbypassMode() || irisIsInLowpowerAbypassMode()) {
		...
		pr_info("iris: reading samsung panel te register c0: 0x%x, 0x%x\n", reg_value, (reg_value>>8)&0xff);

		data = (reg_value>>8)&0xff;
	}
	pr_info("samsung panel c0 value: %d\n", data);
...
u8 mipi_panel_reg_dcs_read(struct dsi_panel *panel, enum dsi_cmd_set_state state, char addr)
{
	...
	const struct mipi_dsi_host_ops *ops = panel->host->ops;
	struct dsi_cmd_desc cmds = {
		{0, MIPI_DSI_DCS_READ, 0, 0, 0,
			1, (const void *)get_data,
			1, (void *)iris_read_cmd_buf},
		1, 0};
	int rc = 0;
	u8 data = 0xff;
...
u8 irisReadPanelTeConfig_KingDisplay(struct dsi_panel *panel, enum dsi_cmd_set_state state)
{
	...
	u8 exit_cmd[2] = {0xfe, 0x00};
	struct dsi_cmd_desc kd_te_default_cmd[] = {
		{{0, MIPI_DSI_DCS_SHORT_WRITE_PARAM, 0, 0, 0,
			sizeof(start_cmd), start_cmd,
			1, iris_read_cmd_buf}, 0, 0},
		{{0, MIPI_DSI_SET_MAXIMUM_RETURN_PACKET_SIZE, 0, 0, 0,
			sizeof(set_size), set_size,
			1, iris_read_cmd_buf}, 1, 0},
		{{0, MIPI_DSI_DCS_READ, BIT(6), 0, 0,
			sizeof(get_0a_cmd), get_0a_cmd,
			1, iris_response_a}, 1, 0},
		{{0, MIPI_DSI_DCS_READ, BIT(6), 0, 0,
			sizeof(get_0b_cmd), get_0b_cmd,
			1, iris_response_b}, 1, 0},
		{{0, MIPI_DSI_DCS_SHORT_WRITE_PARAM, 0, 0, 0,
			sizeof(exit_cmd), exit_cmd,
			1, iris_read_cmd_buf}, 0, 0},
	};
	...
	if (irisIsInAbypassMode() || irisIsInLowpowerAbypassMode()) {
		...
		iris_mipi_reg_read(IRIS_MIPI_TX_ADDR + RD_PACKET_DATA_OFFS, &reg_value);
		pr_info("iris: reading kingdisplay panel te register 0A: 0x%x, 0x%x\n", reg_value, (reg_value>>8)&0xff);
		data_0a = (reg_value>>8)&0xff;

		rc = iris_handover_panel_dcs_cmd(panel, &kd_te_default_cmd[3], state);
		iris_mipi_reg_read(IRIS_MIPI_TX_ADDR + RD_PACKET_DATA_OFFS, &reg_value);
		pr_info("iris: reading kingdisplay panel te register 0B: 0x%x, 0x%x\n", reg_value, (reg_value>>8)&0xff);
...
void irisAdjustPanelTE_Samsung(struct dsi_panel *panel, int bypasscmd, u32 value, enum dsi_cmd_set_state state)
{
	...
	if (value < SAMSUNG_PANEL_TE_D1_MIN) {
		pr_err("irisAdjustPanelTE: invalid d1 value for te tweaking");
		return;
	}

	if ((value & 0xff00) > 0) {
		pr_err("reading panel te configuration: ...");
...
void irisAdjustPanelTE_KingDisplay(struct dsi_panel *panel, int bypasscmd, u32 value, enum dsi_cmd_set_state state)
{
	...
	if (value < 2) {
		pr_err("irisAdjustPanelTE: invalid c0 value for te tweaking");
		return;
	}
...
void irisReportTeCount(int vsync_count)
{
	...
	if (TeN0 == 0) {
		...
		te_period_status = rank_te_period_toolong;
		...
		if (irisIsAllowedMemcTrace())
			pr_err("iris:TE avg: %dus, err:%dus, [r%d %d %d %d(%d)][totalerr:%d %d %d %d][avgerr:%d %d %d %d][p%d %d %d (%d)]\n",
				...
				te_quality,
				timeErr_total_rankbad,
				timeErr_total_rankmax,
				timeErr_total_ranknice,
				timeErr_total_rankperfect,
				timeErr_avg_rankbad,
				timeErr_avg_rankmax,
				timeErr_avg_ranknice,
				timeErr_avg_rankperfect,
				rank_te_period_tooshort,
				rank_te_period_normal,
				rank_te_period_toolong,
				te_period_status);
		...
		if (calibrationExpired >= CALIBRATION_EXPIRED_MAX) {
			panelTeCalibrationRequired = 0;
			pr_err("iris:TE rt: calibration expired: %d\n", deltaTimeErr);
		}
	}
...
void irisMonitorPanelTE(struct dsi_panel *panel)
{
	...
	if ((rank_bad + rank_max + rank_nice + rank_perfect) < RANK_VALID_THRESHOLD
		...
		//pr_info("iris:TE system waiting for more info.\n");
		return;
	}
	...
	if (teIsGoodEnough) {
		...
		if (zte_lcd_module == LCM_SAMSUNG) {
			if (set_te_on_next_wakeup > 0) {
				if (panel_working_c0 != panel_expected_c0) {
					//reset to the same
					panel_expected_c0 = panel_working_c0;
					pr_info("iris:TE abort the te set on next-wakeup due to nice TE quality.");
				}
				set_te_on_next_wakeup = 0;
			}
		}
		...
		if (irisIsAllowedMemcTrace())
			pr_info("iris:TE system TE quality nice(0x%x),period status %d,time-err:%d, no need TE tweaking.\n",
	...
	if (irisIsAllowedMemcTrace())
		pr_info("iris:TE quality level %d,period status %d,time-err:%d need tweaking for MEMC.\n",
	...
	if (zte_lcd_module == LCM_SAMSUNG) {
		if (te_quality < TE_NICE
			&& te_period_status == TE_PERIOD_TOO_LONG) {
			if (panel_working_c0 == panel_expected_c0) {
				if (panel_expected_c0 == 0) {//NOT SET YET
					panel_expected_c0 = SAMSUNG_PANEL_TE_INIT_OTP_VALUE;
				} else {
					//one step is about 0.2Hz, about 56us/step
				...
				pr_info("iris:TE set TE on next-wakeup: 0x%x", panel_expected_c0);
			} else if (set_te_on_next_wakeup <= 0) {
				pr_info("iris:TE should never go here: [0x%x, 0x%x]",
					panel_working_c0, panel_expected_c0);
				return;
			}
		} else if (panel_working_c0 == 0) {
			if (irisIsAllowedMemcTrace())
				pr_info("iris:TE next-wakeup did nothing due to unknown initial c0.");
			return;
		}

		if (irisIsAllowedMemcTrace())
			pr_info("iris:TE next-wakeup: quality %d,period status %d,time-err:%d.[0x%x, 0x%x]",
				te_quality,
				te_period_status,
				deltaTimeErr,
				panel_working_c0,
		...
		if (set_te_on_next_wakeup > 0) {
			if (irisIsAllowedMemcTrace())
				pr_info("iris:TE wait for next-wakeup to set TE");
			return;
		}
	}
#endif
	...
	if ((te_training_started == 0)
		...
		if (irisIsAllowedMemcTrace())
			pr_info("iris:TE system not in FRC-PRE or FRC for TE tweaking: %d\n",
	...
	if (panel_trained_c0 > 0 && panel_expected_c0 == 0) {
		...
		pr_info("iris:TE set expected c0 to the trained one: 0x%x.\n", panel_trained_c0);
	}
#endif
	if (panel_working_c0 == panel_expected_c0) {
		if (zte_lcd_module == LCM_KINGDISPLAY) {
			//one step is about 0.06~0.08Hz, about 17us/step
			int adjustTimeErr = 0;
			int step = 0;
			...
			if (step > ALLOWED_PERFECT_DEVIATION)
				step = step/KINGDISPLAY_TE_ONE_STEP_US;
			else
				step = 0;
...
void iris_abypass_switch_state_init(int mode)
{
	...
	pr_info("iris_abypass_switch_state_init: %d(analog_bypass_enable=%d)\n",
		mode, iris_info.abypss_ctrl.analog_bypass_enable);

	if (!iris_info.abypss_ctrl.analog_bypass_enable)
		return;

	if (iris_abyp_gpio_init(iris_panel) != 0) {
		pr_err("%s abyp gpio init err!\n", __func__);
		return;
	}
...
u8 iris_mipi_signal_mode_read(struct dsi_panel *panel, enum dsi_cmd_set_state state)
{
	...
	const struct mipi_dsi_host_ops *ops = panel->host->ops;
	struct dsi_cmd_desc cmds = {
		{0, MIPI_DSI_DCS_READ, 0, 0, 0, sizeof(get_signal_mode), get_signal_mode, 1, iris_read_cmd_buf}, 1, 0};
	int rc = 0;
	u8 data = 0xff;
...
int iris_abyp_gpio_init(struct dsi_panel *panel)
{
	...
	if (gpio_direction_input(panel->dsp_cfg.dsp_wakeup) != 0) {
...
#define POR_CLOCK2 180	/* 0.1 Mhz*/
	...
	if (!gpio_is_valid(iris_panel->dsp_cfg.dsp_gpio_test)) {
	...
	if (round == 1) {
		gpio_direction_output(iris_panel->dsp_cfg.dsp_gpio_test, 0);

		/* added this api into drivers/pinctrl/qcom/pinctrl-msm.c */
		/* oem_gpio_control(iris_panel->dsp_cfg.dsp_gpio_test); */
		pr_err("gpio:%d\n", iris_panel->dsp_cfg.dsp_gpio_test);
	} else {
		spin_lock_irqsave(&iris_info.iris_lock, flags);
		gpio_direction_output(iris_panel->dsp_cfg.dsp_gpio_test, 0);
		gpio_set_value(iris_panel->dsp_cfg.dsp_gpio_test, 0);
		ktime0 = ktime_get();
		gpio_set_value(iris_panel->dsp_cfg.dsp_gpio_test, 1);
		ktime1 = ktime_get();
		gpio_set_value(iris_panel->dsp_cfg.dsp_gpio_test, 0);
		ktime2 = ktime_get();
		gpio_set_value(iris_panel->dsp_cfg.dsp_gpio_test, 1);
...
void iris_low_power_mode_set(struct dsi_panel *panel, bool enable, enum dsi_cmd_set_state state)
{
	...
	if (enable) {
		...
		*(u16 *)(grcp_cmd.cmd + 14) = cpu_to_le16(grcp_len);
		...
		pr_info("iris low power mode: %d(packet len %d) line 3185\n",
		...
		iris_cmd_reg_add(&grcp_cmd,
				...
				clock->mcuclk.sel | (clock->mcuclk.div << 8) | (clock->mcuclk.div_en << 12));
		iris_cmd_reg_add(&grcp_cmd,
	...
	*(u16 *)(grcp_cmd.cmd + 14) = cpu_to_le16(grcp_len);
	...
	pr_info("iris low power mode: %d(packet len %d)line 3254\n",
		enable, (int)iris_dsi_format_cmd.msg.tx_len);
...
int iris_pt_to_abypass_switch(struct dsi_panel *panel)
{
	...
	case LOW_POWER_ENTER_STATE:
		gpio_value = gpio_get_value(panel->dsp_cfg.dsp_wakeup);
...
static struct dsi_ctrl *iris_get_dsi_ctrl(void)
{
	...
	if (display->ctrl_count > 1) {
		pr_err("%s ctrl count is not right = %d\n", __func__, display->ctrl_count);
		return NULL;
	}
...
bool iris_sde_encoder_rc_lock(void)
{
	...
	/*ctrl_lock will be needed by transfer_msg*/
