...
void iris_cmlut_table_load(struct dsi_panel *panel)
{
	...
	if (chip_setting->cm_setting.color_temp_en) {
		pr_err("iris_cmlut_table_load:  cct sensor-en %d, chip cct=%d,temp_value(%d)\n",
			(int)chip_setting->cm_setting.sensor_auto_en,
			(int)chip_setting->cct_value.cctvalue,
			(int)temp_value);
...
void iris_gamma_table_update(struct feature_setting *chip_setting)
{
	...
	if (!iris_info.gamma_info.gamma_fw_state || !gamma_fw_buf) {
		pr_err("iris_gamma_table_update: ccf2 not ready.\n");
		return;
	}
	...
	//iris_gamma_table_reg_add(0xf1580000,
	//Gamma_size + (dither_mode << 2) + (((width == 12) ? 0 : 1) << 15), false);
...
void iris_mipi_reg_write(u32 addr, u32 value, enum dsi_cmd_set_state state)
{
	u32 grcp_len = 0;
		...
		*(u16 *)(grcp_cmd.cmd + 14) = cpu_to_le16(grcp_len);
		...
		pr_debug("iris: iris_mipi_reg_write reg(0x%x), value(0x%x)\n", addr, value);
}
...
int iris_mipi_reg_read(u32 addr, u32 *value)
{
	...
	*(u32 *)(grcp_cmd.cmd + 12) = cpu_to_le32(addr);
	...
	pi_read_cmd.msg.rx_buf = iris_read_cmd_buf;
		...
		pr_debug("mipi read msg.tx_len=%d, msg.rx_len=%d, iris reg(%x), rc=%d, value=%x\n",
		(int)pi_read_cmd.msg.tx_len, (int)pi_read_cmd.msg.rx_len, addr, rc, response);
...
void iris_cmlut_table_buf_update(struct dsi_panel *panel, u32 cm3d)
{
	...
	if (!lut_info->lut_fw_state) {
		pr_err("iris_cmlut_table_buf_update: ccf not ready.\n");
		return;
	}

	pr_err("iris_cmlut_table_*buf*_update: update CM 3D LUT: %d.\n", (int)cm3d);
...
void iris_cmlut_table_update(struct dsi_panel *panel, struct feature_setting *chip_setting)
{
	...
	if (!lut_info->lut_fw_state) {
		pr_err("iris_cmlut_table_update: ccf not ready.\n");
		return;
	}

	if (chip_setting->hdr_setting.hdren) {
		//pr_err("iris_cmlut_table_update: update HDR LUT.\n");
		iris_cmlut_grcp_set(panel, CMI_HDR_6500_C51_ENDIAN, (u32)IRIS_CMLUT_IP_ADDR);
	} else {
		if (chip_setting->cm_setting.color_temp_en) {
			//pr_err("iris_cmlut_table_update: update CCT LUT: sensor-en %d.chip cct %d,temp_value %d\n",
				//(int)chip_setting->cm_setting.sensor_auto_en,
				//(int)chip_setting->cct_value.cctvalue,
				//(int)temp_value);
			if (chip_setting->cm_setting.sensor_auto_en) {
				...
				iris_cmlut_color_temp_set(chip_setting->cct_value.cctvalue,
							chip_setting->cm_setting.cm3d);
				iris_cmlut_grcp_set(panel, CMI_CALCULATION, (u32)IRIS_CMLUT_IP_ADDR);
			}
		} else {
			//pr_err("iris_cmlut_table_update: update CM 3D LUT: %d.\n",
...
static u32 iris_CCT_remapping(u32 sensor_cct)
{
	...
	pr_err("ori_cct = %d, wanted = %d\n", sensor_cct, wanted_cct);
	return wanted_cct;
}
...
int iris_configure(u32 type, u32 value)
{
	...
	if (irisIsFullyFunctioning()) {/*avoid interrupting iris lightup*/
		if (settint_update->lux_value) {
			...
			iris_mipi_reg_write(IRIS_LUX_VALUE_ADDR,
						user_setting->lux_value.luxvalue,
			...
			iris_mipi_reg_write(IRIS_CCT_VALUE_ADDR,
						user_setting->cct_value.cctvalue,
						DSI_CMD_SET_STATE_HS);
			settint_update->cct_value = false;
		}
	} else {
		pr_info("iris_configure:iris is NOT in full functioning.\n");
		/*saved the latest cct and lux*/
		if (settint_update->lux_value)
			chip_setting->lux_value = user_setting->lux_value;
		else if (settint_update->cct_value)
			chip_setting->cct_value = user_setting->cct_value;
...
int iris_low_power_process(void)
{
	...
	lp_switch = (*state == LP_STATE_POWER_UP_PREPARE)
				|| (*state == LP_STATE_POWER_DOWN_PREPARE);

	mode_switch = mode_state->sf_mode_switch_trigger
		&& ((mode_state->sf_notify_mode == IRIS_MODE_FRC_PREPARE)
	...
	if (lp_switch || mode_switch || cm_switch) {
...
#else
		//iris_mipi_reg_read(IRIS_MIPI_RX_ADDR + DCS_CMD_PARA_2, &val);
		...
		if (signal_mode)
			return false;
		if (*state == LP_STATE_POWER_UP_PREPARE) {
			*state = LP_STATE_POWER_UP;
			//pr_err("lp state = %d\n", *state);
			return true;
		} else if (*state == LP_STATE_POWER_DOWN_PREPARE) {
			*state = LP_STATE_POWER_DOWN;
		}
		if ((mode_switch || cm_switch) && (*state == LP_STATE_POWER_DOWN)) {
#ifdef I2C_ENABLE
...
void iris_low_power_mode_notify(void)
{
	...
	if (!(((mode_state->current_mode == IRIS_RFB_MODE) && (mode_state->sf_notify_mode == IRIS_MODE_RFB))
		|| ((mode_state->current_mode == IRIS_FRC_MODE) && (mode_state->sf_notify_mode == IRIS_MODE_FRC))))
		return;
...
#endif
	*state = LP_STATE_POWER_DOWN_PREPARE;
	pr_err("enter low power mode: current_mode=%d, sf_notify_mode=%d\n",
		mode_state->current_mode, mode_state->sf_notify_mode);
}
...
void iris_cmd_kickoff_proc(void)
{
	...
	if (iris_info.power_status.low_power_state != iris_power_prev_st) {
		pr_info("%s: iris power_state: %d ->% d\n",
			__func__,
			iris_power_prev_st,
			iris_info.power_status.low_power_state);
		iris_power_prev_st = iris_info.power_status.low_power_state;
	}

/*modify by zte for power off status send command, cause pc mode lcd can't lightup*/
	if (!iris_panel->panel_initialized) {
		pr_err("%s kickoff error when panel power down\n", __func__);
		return;
	}
/*modify by zte for power off status send command cause pc mode lcd can't lightup*/
	...
	if (!record_first_kickoff) {
		...
		//pr_info("%s perf-check: first kickoff at %dus\n", __func__, timeus01);
	}
