					 ...
					 char __user *buffer, size_t count, loff_t *offset)
{
	...
	pr_notice("tpd: tp_rawdata_read:%lld\n", *offset);
	if (*offset != 0) {
		return 0;
	}
	mutex_lock(&cdev->cmd_mutex);
...
static void create_tpd_proc_entry(void)
{
	...
	tpd_proc_entry = proc_create("smart_cover", 0664, tpd_proc_dir, &proc_ops_smart_cover);
	if (tpd_proc_entry == NULL)
		pr_notice("proc_create smart_cover failed!\n");
	tpd_proc_entry = proc_create("rw_reg", 0664,  tpd_proc_dir, &proc_ops_rwreg);
	if (tpd_proc_entry == NULL)
		pr_notice("proc_create rw_reg!\n");
	tpd_proc_entry = proc_create("get_tprawdata", 0664,  tpd_proc_dir, &proc_ops_tprawdata_read);
	if (tpd_proc_entry == NULL)
		pr_notice("proc_create tprawdata!\n");
}
