	...
	int *p_fw_ver)
{
	...
	case 'U':
		...
		break;
	case 'X':
		snprintf(p_chip_type, PAGE_SIZE, "TD4310(0x%x)", ts->config_id.chip_type);
		break;
	case 'b':
		snprintf(p_chip_type, PAGE_SIZE, "S3706(0x%x)", ts->config_id.chip_type);
		break;
	default:
	...
	case 'E':
		...
		break;
	case 'F':
		snprintf(p_sensor, PAGE_SIZE, "BOE(0x%x)", ts->config_id.sensor);
		touch_module = BOE;
		break;
	case 'H':
	...
	case 'R':
		...
		break;
	case 'V':
		snprintf(p_sensor, PAGE_SIZE, "GVO(0x%x)", ts->config_id.sensor);
		touch_module = GVO;
		break;
	default:
		...
		bool enable)
{
	...
	if (enable) {
		...
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to diable detect smart cover, retval = %d\n",
				__func__, retval);
			return;
		}
